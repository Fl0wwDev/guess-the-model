...
- **Showcase / Museum** (a core future vision, beyond the quiz): an elegant, immersive **brand museum** — the user strolls through a manufacturer (e.g. Ferrari) and its **entire history**. Each model card carries full specs (horsepower, French *chevaux fiscaux*, release year, **sales figures**), **anecdotes** and story. A guided, cinematic journey through the marque and its timeline — not just a flat catalog. Reference/inspiration: vecarz.com (dark editorial, serif headings, embedded 3D, rich spec sheet).
  - **The museum renders 3D via Sketchfab embeds, NOT local GLB.** Sketchfab streams at full quality on their infra → zero local optimization, zero lag, and card thumbnails come free from their CDN. Models come from **Ddiaz Design**'s per-brand "base models" collections. Mapping is generated by `npm run sync:sketchfab -- <brandId> <collectionUid>` → `src/content/sketchfab/<brand>.json` (keyed by **variant id**; matched to our registry by normalized name). Loader: `src/content/sketchfab.ts`. Trade-offs accepted: needs internet + Sketchfab uptime; link-rot if an author deletes a model.
  - Museum routes: `/musee` (brand grid) → `/musee/[brand]` (model grid, chronological, Sketchfab thumbnails) → `/musee/[brand]/[model]` (editorial detail: `<SketchfabEmbed>` hero + serif title + variant switcher + spec sheet + story + related models). Specs are hand-authored in `src/content/specs/<brand>.json` (never generated).
