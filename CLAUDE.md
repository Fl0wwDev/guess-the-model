...
- **Showcase / Museum** (a core future vision, beyond the quiz): an elegant, immersive **brand museum** — the user strolls through a manufacturer (e.g. Ferrari) and its **entire history**. Each model card carries full specs (horsepower, French *chevaux fiscaux*, release year, **sales figures**), **anecdotes** and story. A guided, cinematic journey through the marque and its timeline — not just a flat catalog. Reference/inspiration: vecarz.com (dark editorial, serif headings, embedded 3D, rich spec sheet).
  - **Full Sketchfab: the museum catalogue is built DIRECTLY from Ddiaz Design's Sketchfab collections — no local GLB.** Sketchfab streams at full quality on their infra → zero local optimization, zero lag, thumbnails free from their CDN, 100% coverage by construction. `npm run build:museum` (`scripts/build-museum.mjs`) reads `src/content/museum/collections.json` (brand → collection uid, + `modelKeys` hints for brands with no local seed like BMW), fetches each collection, **groups models** (seeded from the folder registry `cars.ts` so ids stay stable → hand-authored specs keep resolving; token-based key match; first-token fallback), picks a base variant, and writes `src/content/museum/<brand>.json`. Loader + helpers: `src/content/museum.ts` (`MUSEUM_BRANDS`, `getMuseumBrand/Model`, `museumBaseVariant`). To add a brand: add its collection uid to the config + rerun. Trade-offs accepted: needs internet + Sketchfab uptime; link-rot if an author deletes a model.
  - The museum is **decoupled from `cars.ts`** — that folder registry now only feeds the landing garage (and a future local quiz). The `<SketchfabEmbed>` uses the **Sketchfab Viewer API** (client-side) to pull the camera back on load (models framed too tight otherwise).
...
## 3D asset pipeline (local GLB — QUIZ only now)

**Scope note:** the museum uses **Sketchfab embeds** (see Project above), so this local-GLB pipeline is now only for the **Quiz** — which needs local, offline, deterministic models with **no visible model name** (a Sketchfab embed shows the title → spoils the answer). The 2.7 GB in `public/models/` is kept for the quiz; the registry (`src/lib/cars.ts`, base-variant logic) stays the backbone for both features.
...
- Load via drei `useGLTF` + `<Suspense>`; `useGLTF.preload()` the next car; frame with `<Bounds>`. Register meshopt/KTX2 decoders once; self-host the wasm under `public/decoders/`. Shared rig in `src/components/three/Studio.tsx`.

## Build & tooling gotchas (hard-won)

- **Tailwind v4 must be scoped to `src/`.** `globals.css` uses `@import "tailwindcss" source("../");`. Without the `source(...)`, Tailwind v4's automatic content detection walks the **whole project root — including the multi-GB `public/models` tree — and hangs the first compile indefinitely** (dev AND `next build`, both Turbopack and webpack; every route stalls because the CSS is in the root layout). Symptom: `next-swc` pegs 2–3 cores with a runaway memory footprint, stuck on "Creating an optimized production build" / "Compiling …". Never remove the `source("../")` scope. `public/models/` is also gitignored (correct hygiene; not sufficient on its own).
